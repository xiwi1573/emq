...
Ext.extend(Ext.ux.grid.ProgressColumn, Ext.util.Observable, {
  /**
   * @cfg {String} colored determines whether use special progression coloring or the standard
   *      Ext.ProgressBar coloring for the bar (defaults to false)
   */
  colored : false,
  /**
  ...
  renderer : function(v, p, record) {
    var style = '';
    var text_class = (v < 55) ? 'x-progress-text-back' : 'x-progress-text-front' + (Ext.isIE6 ? '-ie6' : '');
    ...
    return String.format('' + '<div class="x-progress-wrap"><div class="x-progress-inner">'
        + '<div class="x-progress-bar{0}" style="width:{1}%;"></div>'
        + '<div class="x-progress-text {2}" style="width:100%;" id="{4}">{3}</div></div></div>', style, v, text_class, v + '%', Ext
        .id());
  }
});