...
Ext.ux.grid.icss.LockingGridView = function(config) {
    ...
    Ext.ux.grid.icss.DynamicGridView.superclass.constructor.call(this);
};
Ext.extend(Ext.ux.grid.icss.LockingGridView, Ext.ux.grid.icss.DynamicGridView, {
    lockText : "Lock",
    unlockText : "Unlock",
   ...
   setSumValue:function(dataIndex,v){
    ...
    scrollToTop : function() {
        Ext.ux.grid.icss.LockingGridView.superclass.scrollToTop.call(this);
        this.syncScroll();
    },

    syncScroll : function(e) {
        Ext.ux.grid.icss.LockingGridView.superclass.syncScroll.call(this, e);
    ...
    onColumnMove : function(cm, oldIndex, newIndex) {
        Ext.ux.grid.icss.LockingGridView.superclass.onColumnMove.call(this, cm, oldIndex, newIndex);
        this.changeHeight();
    },
    ...
    doRender : function(cs, rs, ds, startRow, colCount, stripe) {
		...
		for (var i = 0; i < colCount; i++) {
//			if(this.refreshHeader?false:this.renderHeader){
//				break;
//			}
			c = cs[i];
			//处理求和问题
			if(true == this.cm.sumheader){
				if(true == this.cm.config[i].issum){
				   for(var j=0;j<ds.getTotalCount();j++){
						if(this.cm.config[i].sfn){//如果存在求和函数，则调用
							this.cm.config[i].sumvalue = this.cm.config[i].sfn(this.cm.config[i].sumvalue,r.get(c.name),r);
						}else{//否则，直接累计
							if(typeof(ds.getAt(j).get(c.name)) == 'number'){
								this.cm.config[i].sumvalue += ds.getAt(j).get(c.name);
							}else{
								this.cm.config[i].sumvalue += 0;
							}
						}
				   }
				}
        ...
        //如果有合计行，则需要重新渲染表头
//        if(true == this.hasSumColumn && (this.refreshHeader?true:!this.renderHeader)){
	...
	*/
    ...
    renderUI : function() {
		...
		this.scroller.on('mousewheel',this.syncMousewheel,this);
		this.scroller.on('resize',this.onLiveScroll,this);
	...
	*/
    ...
    insertRows : function(dm, firstRow, lastRow, isUpdate) {
        if (firstRow === 0 && lastRow == dm.getCount()-1) {
            ...
            var before = this.getRow(firstRow-vr.first);
            if (before) {
            ...
            var beforeLocked = this.getLockedRow(firstRow-vr.first);
            if (beforeLocked) {
    ...
    renderBody : function() {
		var vr = this.getVisibleRows();
        var markup = this.renderRows(vr.first,vr.last);
